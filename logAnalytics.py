    ...
    sql = '''
        ...
        FROM log
            LEFT JOIN articles art
                ON art.slug = substring(path, 10, length(log.path))
...
# the most page views? Present this as a sorted list with the most
...
def get_days_with_more_than_one_percent_errors():
    ...
    sql = '''
        SELECT
            ...
            SUM(CASE WHEN log.status <> '200 OK' THEN 1 ELSE 0 END)
                *100::decimal/COUNT(1) as per_error
        FROM log
            LEFT JOIN articles
                ON articles.slug = substring(path, 10, length(log.path))
            LEFT JOIN authors ON authors.id = articles.author
        WHERE path != '/'
        GROUP BY
            DATE(log.time)
        HAVING SUM(CASE WHEN log.status <> '200 OK' THEN 1 ELSE 0 END)
                *100::decimal/COUNT(1) >= 1
        ORDER BY view_cnt DESC
        ;
    '''
    c.execute(sql)
    ...
    for date_day, error_cnt, view_cnt, per_error in data:
        print "{} - {:.2f}% errors".format(
            date_day.strftime("%B %d, %Y"), per_error)
